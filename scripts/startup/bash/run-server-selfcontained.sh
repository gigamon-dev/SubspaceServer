...
#
# This variant runs the native apphost directly (./bin/SubspaceServer) and does NOT
# require the .NET runtime to be installed. For framework-dependent builds, use run-server.sh.
#
...
if [ ! -f "$ZONEDIR/bin/SubspaceServer" ]; then
  echo "Directory '$ZONEDIR' is not a valid self-contained zone directory ('bin/SubspaceServer' not found)."
  exit 1
fi
...
while $CONTINUE; do
  ...
  ./bin/SubspaceServer 3>&1 1>&2 2>&3 | tee -a $SCRIPTLOG
